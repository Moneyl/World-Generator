...
// For each octave, a higher frequency/lower amplitude function will be added to the original.
// The higher the persistence [0-1], the more of each succeeding octave will be added.
float octave_noise_3d( const float octaves, const float persistence, const float scale, const float aAmplitude, const float x, const float y, const float z )
{
float total = 0;
float frequency = scale;
float amplitude = aAmplitude;
...
//
// Returned value will be between loBound and hiBound.
float scaled_octave_noise_3d( const float octaves, const float persistence, const float scale, const float aAmplitude, const float loBound, const float hiBound, const float x, const float y, const float z ) {
return octave_noise_3d(octaves, persistence, scale, aAmplitude, x, y, z) * (hiBound - loBound) / 2 + (hiBound + loBound) / 2;
}
